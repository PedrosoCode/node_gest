...
const loginUsuario = async (req, res) => {
  ...
  try {
    ...
    const user = await pool.query('SELECT * FROM tb_cad_usuario WHERE email = $1 AND codigo_empresa = $2', [email, codigo_empresa]);
...
const obterUsuario = async (req, res) => {
  try {
    ...
    const usuario = await pool.query('SELECT codigo, usuario, email, codigo_empresa FROM tb_cad_usuario WHERE codigo = $1', [userId]);
